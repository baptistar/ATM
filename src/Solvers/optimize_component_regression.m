function [S, exit_flag] = optimize_component_regression(S, Sref, a0, XW, precomp, alpha, coeff_idx)
    ...
    obj = @(a) regression_objective_c(S, Sref, a, A, XW, precomp, alpha, coeff_idx);
...
function [L, dcL] = regression_objective_c(S, Sref, a, A, XW, precomp, alpha, coeff_idx)

    % extract current coeffs
    a_old = S.coeff;
    % pre-condition inputs using matrix 
    ...
    if (nargout == 1)
        L = regression_objective(S, Sref, XW, precomp, coeff_idx);
    else
        [L, dcL] = regression_objective(S, Sref, XW, precomp, coeff_idx);
    end
