classdef ConstExtPhysicistHermiteFunction

    % ConstExtPhysicistHermiteFunction class defines the
    % physicist Hermite functions given by 
    ...
    methods
        function CHF = ConstExtPhysicistHermiteFunction(varargin)
            ...
            CHF.Hf = PhysicistHermiteFunction();

        end %endFunction
        %------------------------------------------------------------------
        %------------------------------------------------------------------
        function p = evaluate(CHF, x, m, norm)
        % evaluate the m-th order Hermite Physicists' function basis
        ...
        % Physicists' functions extended by the constant function
        %
        % Inputs:  x - (N x 1) input samples
        %          m - order of Hermite functions
        ...
        % Hermite Physicists' functions evaluated at N points in x
        %
        % Inputs:  x - (N x 1) input samples
        %          m - order of Hermite functions
        %          k - k-th derivative (k=0 just evaluates poly)
