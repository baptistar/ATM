classdef LinearExtPhysicistHermiteFunction

	% LinearExtPhysicistHermiteFunction class defines the 
	...
	methods
		function LHF = LinearExtPhysicistHermiteFunction(varargin)
			...
			LHF.Hf = PhysicistHermiteFunction();

		end %endFunction
		%------------------------------------------------------------------
		%------------------------------------------------------------------
		function p = evaluate(LHF, x, m, norm)
		% evaluate the m-th order Hermite physicist functions by
		...
		% physicist functions extended by the constant & linear function
		%
		% Inputs:  x - (N x 1) input samples
		% 		   m - order of Hermite functions
