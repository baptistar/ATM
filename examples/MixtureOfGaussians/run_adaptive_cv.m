...
function [CM, output] = buildComposedMap(Xtrain, Xvalid)
    ...
    for k=1:d
        ...
        for fold=1:n_folds
            Sfold = S_validk;
            fprintf('Fold %d/%d\n', fold, n_folds);
            Ztrainkfold.X = Ztrain(cv.training(fold),1:k); 
            Ztrainkfold.W = (1/sum(cv.training(fold)))*ones(sum(cv.training(fold)),1);
            Zvalidkfold.X = Ztrain(cv.test(fold),1:k); 
            Zvalidkfold.W = (1/sum(cv.test(fold)))*ones(sum(cv.test(fold)),1);
            [~, options] = greedy_fit(Sfold, ref.factors{k}, ...
                max_iter, Ztrainkfold, Zvalidkfold, max_patience);
        ...
        fprintf('Final run\n');
        Ztraink.X = Ztrain(:,1:k);
        Ztraink.W = (1/size(Ztrain,1))*ones(size(Ztrain,1),1);
        Zvalidk.X = Zvalid(:,1:k);
        Zvalidk.W = (1/size(Zvalid,1))*ones(size(Zvalid,1),1);
        [PB.S.S{k}, output{k}] = greedy_fit(PB.S.S{k}, ref.factors{k}, ...
            opt_terms, Ztraink, Zvalidk, []);
