function [S, output] = adaptive_transport_map(S, ref, Xtrain, Xvalid, ...
    max_iterations, max_patience, var_tol, alpha)
...
if ~isa(Xvalid,'struct')
    XWvalid.X=Xvalid;
    XWvalid.W=(1/size(Xvalid,1))*ones(size(Xvalid,1),1);
else
    XWvalid=Xvalid;
end
...
max_iterations = min(max_iterations, size(XWtrain.X,1));
% set validation samples
if (nargin < 5) || isempty(XWvalid)
    XWvalid.X = zeros(0, size(XWtrain.X,2));
    XWvalid.W=(1/size(XWvalid.X,1))*ones(size(XWvalid.X,1),1);
end
% check validation samples
if max_patience < Inf && size(XWvalid.X,1) == 0
...
precomp_Xtrain = set_precomp(S, XWtrain.X);
precomp_Xvalid = set_precomp(S, XWvalid.X);

% measure generalization error on all data sets with initial map
[S,~] = optimize_map(S, ref, S.coeff, XWtrain, precomp_Xtrain, alpha);
train_err = negative_log_likelihood(S, ref, XWtrain, precomp_Xtrain);
valid_err = negative_log_likelihood(S, ref, XWvalid, precomp_Xvalid);
rho=IndependentProductDistribution(repmat({Normal()},1,S.d));
var_d=log10(var_diag(rho,PullbackDensity(S,ref),XWtrain));
...
% define output_msg
output_msg = 'Term %4d - Training error: %8.4f, Validation error: %8.4f, log-Variance diag: %8.4f\n';
fprintf(output_msg, S.n_coeff, train_err, valid_err,var_d);

%% Greedy procedure

while((S.n_coeff < max_iterations && patience < max_patience && var_d(end)> var_tol) )
    ...
    [S_new, new_midx] = update_map(S, ref, reduced_margin, XWtrain, precomp_Xtrain);
    
    % update precomputed structures
    precomp_Xtrain = update_precomp(precomp_Xtrain, S_new, XWtrain.X, new_midx);
    precomp_Xvalid = update_precomp(precomp_Xvalid, S_new, XWvalid.X, new_midx);
    ...
    if flag <= -1
        ...
        var_d=[var_d,nan*ones(1,max_terms-S.n_coeff)];
        break
    else
        ...
        var_d = [var_d, log10(var_diag(rho,PullbackDensity(S,ref),XWtrain))];
        List_map=[List_map,S];
    end
    
    % print output
    fprintf(output_msg, S.n_coeff, train_err(end), valid_err(end),var_d(end));
...
function [S, new_midx] = update_map(S, ref, reduced_margin, XW, precomp)
...
[~, dJ] = negative_log_likelihood(S_margin, ref, XW, precomp_margin);
...
function [L, dcL] = negative_log_likelihood(S, ref, XW, precomp)
